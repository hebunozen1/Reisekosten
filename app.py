...
def login():
    ...
    if request.method == "POST":
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT u.startguthaben_sar,
        id, username, email, password_hash, role FROM users WHERE LOWER(email) = LOWER(%s)"
    ...
    if role == "buchhaltung":
        ...
        cur.execute(
            ...
            SELECT k.id, k.user_id, u.username, k.datum, k.kategorie_ar, k.beschreibung_ar, k.betrag_sar,
                   k.beleg, k.status_text, k.genehmigt_von
            FROM kosten k
...
def export_excel():
    ...
    writer = csv.writer(output, delimiter=';')
        ...
        writer.writerow([
        "ID", "Reiseführer", "Datum", "Kategorie", "Beschreibung (AR)", "Betrag (SAR)", "Status"])
    for r in rows:
        if hasattr(r, "keys"):
