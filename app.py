...
@login_required
def delete_guide(user_id):
    if current_user.role != "buchhaltung":
        abort(403)
    ...
    # delete user
    cur.execute(
        "DELETE FROM users WHERE id = %s AND role = 'reisefuehrer'"
        if DATABASE_URL else
        "DELETE FROM users WHERE id = ? AND role = 'reisefuehrer'",
        (user_id,)
    )
    ...
    flash("Reiseführer und alle Ausgaben wurden gelöscht.", "success")
    return redirect(url_for("buchhalter_dashboard"))
