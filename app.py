...
def login():
    # role highlighting in UI (default: Reiseführer)
    role = (request.args.get("role") or request.form.get("role") or "reisefuehrer").strip().lower()
    if role not in ("reisefuehrer", "buchhaltung"):
        role = "reisefuehrer"
    ...
    if request.method == "POST":
        ...
        flash("Login fehlgeschlagen. Bitte E-Mail/Passwort prüfen.", "err")
