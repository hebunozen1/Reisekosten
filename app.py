...
def get_today_date():
    """Return the app's current date. Used for date limits in expense entry."""
    return datetime.utcnow().date()


def validate_expense_date(date_text):
    """Allow Reiseführer to create expenses only for today or max. 7 days back."""
    try:
        expense_date = datetime.strptime((date_text or "").strip(), "%Y-%m-%d").date()
    except ValueError:
        return False, "Bitte ein gültiges Datum auswählen."

    today = get_today_date()
    earliest_allowed = today - timedelta(days=7)

    if expense_date < earliest_allowed:
        return False, f"Ausgaben dürfen nur für heute oder maximal 7 Tage rückwirkend eingetragen werden. Frühestes erlaubtes Datum: {earliest_allowed.strftime('%d.%m.%Y')}."
    if expense_date > today:
        return False, "Ausgaben mit zukünftigem Datum dürfen nicht eingetragen werden."
...
def get_env_var(name: str, default: str = "") -> str:
    """Read an environment variable robustly, including accidental whitespace/case variants."""
    value = os.environ.get(name)
    ...
    for key, value in os.environ.items():
        if key.strip().lower() == wanted and value is not None and str(value).strip():
            return str(value).strip()

    return default


def log_smtp_env_status(context: str):
    required = ["SMTP_HOST", "SMTP_PORT", "SMTP_USER", "SMTP_PASSWORD", "SMTP_FROM", "RESET_BASE_URL", "PUBLIC_BASE_URL"]
    present = {name: bool(get_env_var(name)) for name in required}
    smtp_like_keys = sorted([key for key in os.environ.keys() if "SMTP" in key.upper() or "BREVO" in key.upper() or key.upper() in ("RESET_BASE_URL", "PUBLIC_BASE_URL")])
    print(f"{context}: env_present={present}; smtp_like_keys={smtp_like_keys}", flush=True)


def build_reset_link(token: str) -> str:
    # In Render ist bei dir RESET_BASE_URL gesetzt. PUBLIC_BASE_URL bleibt als Fallback.
    base_url = (get_env_var("RESET_BASE_URL") or get_env_var("PUBLIC_BASE_URL")).rstrip("/")
    if base_url:
...
def send_smtp_message(to_email, subject, body):
    ...
    raw_port = get_env_var("SMTP_PORT", "587")
    try:
        port = int(raw_port)
    except ValueError:
        raise RuntimeError(f"SMTP_PORT ist ungültig: {raw_port!r}")

    user = get_env_var("SMTP_USER")
    password = get_env_var("SMTP_PASSWORD")
    from_email = get_env_var("SMTP_FROM") or user
    ...
    if missing:
        log_smtp_env_status("SMTP CONFIG ERROR")
        raise RuntimeError(
    ...
    print(f"SMTP: connecting to {host}:{port} ssl={use_ssl} tls={use_tls}", flush=True)
    server = None
    try:
        if use_ssl:
        ...
        print("SMTP: mail sent", flush=True)
    finally:
...
def send_registration_request_email(pending_user_email, pending_username, requested_role):
    ...
    approval_link = (get_env_var("RESET_BASE_URL") or get_env_var("PUBLIC_BASE_URL")).rstrip("/") or request.host_url.rstrip("/")
    body = f"""
Hallo,
...
def send_registration_approved_email(to_email, username, role):
    ...
    login_link = (get_env_var("RESET_BASE_URL") or get_env_var("PUBLIC_BASE_URL")).rstrip("/") or request.host_url.rstrip("/")
    body = f"""
Hallo {username},
...
def forgot():
    if request.method == "POST":
        ...
        try:
            ...
            user = cur.fetchone()
            print(f"FORGOT_PASSWORD: request received for {email}; user_found={bool(user)}", flush=True)
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if action == "add_kosten":
                ...
                date_ok, date_error = validate_expense_date(datum)
                if not date_ok:
                    flash(date_error, "err")
                    return redirect(url_for("dashboard", monat=request.args.get("monat", ""), reise=request.args.get("reise", "")))
    ...
    try:
        ...
        return render_template(
            ...
            expense_min_date=(get_today_date() - timedelta(days=7)).strftime("%Y-%m-%d"),
            start=start_amount,
...
def buchhalter_delete_guide(user_id):
    # Sicherheits-Sperre: Reiseführer dürfen von der Buchhaltung nicht mehr gelöscht werden.
    # Die Route bleibt bestehen, damit alte Browser-Tabs oder direkte POST-Anfragen
    # sauber blockiert werden und keine Daten löschen können.
    if "user_id" not in session:
        flash("Bitte zuerst einloggen.", "error")
        return redirect(url_for("login"))

    flash("Reiseführer können nicht mehr von der Buchhaltung gelöscht werden.", "error")
    return redirect(url_for("dashboard"))
