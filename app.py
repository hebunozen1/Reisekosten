...
def send_reset_email(to_email: str, reset_url: str) -> None:
    """Send a password-reset email via SMTP (e.g., STRATO)."""
    ...
    if not smtp_user or not smtp_password or not smtp_from:
        raise RuntimeError("SMTP credentials are not configured (SMTP_USER/SMTP_PASSWORD/SMTP_FROM).")
    ...
    # Plain-text email (works everywhere). You can add HTML later if you want.
    msg.set_content(
        "Hallo,\n\n"
        "du hast ein Zurücksetzen deines Passworts angefordert.\n"
        f"Klicke auf diesen Link, um ein neues Passwort zu setzen:\n{reset_url}\n\n"
...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY") or os.environ.get("APP_SECRET_KEY", "change-me-in-production")
...
def _adapt_sql(sql: str) -> str:
    """Keep the existing code's SQLite-style '?' placeholders and adapt for Postgres."""
...
class _DB:
    def __init__(self, con, dialect: str):
        self.con = con
    ...
    def executemany(self, sql: str, seq_of_params):
        ...
        if self.dialect == "postgres":
            ...
            return self.con.executemany(sql, seq_of_params)
...
def get_db() -> _DB:
    """Returns a lightweight DB wrapper with .execute/.commit/.close."""
    if DB_DIALECT == "postgres":
        if psycopg2 is None:
            raise RuntimeError("psycopg2 is required for PostgreSQL but is not installed.")
        # Render provides DATABASE_URL. sslmode=prefer works both locally and on Render.
        con = psycopg2.connect(DATABASE_URL, sslmode=os.environ.get("PGSSLMODE", "prefer"))
        return _DB(con, "postgres")

    con = sqlite3.connect("reisekosten.db")
    con.row_factory = sqlite3.Row
    return _DB(con, "sqlite")
...
def ensure_schema() -> None:
    ...
    # Ensure there is a buchhaltung account (initial setup). You should change this for production.
    row = db.execute("SELECT 1 FROM users WHERE role='buchhaltung' LIMIT 1").fetchone()
    if not row:
        db.execute(
            "INSERT INTO users (username, email, password_hash, role) VALUES (?,?,?,?)",
            ("alaa", "buchhaltung@local", generate_password_hash("alaa123"), "buchhaltung"),
        )
...
# Initialize schema once at startup (works for Gunicorn/Render)
try:
    ensure_schema()
except Exception as _e:
    # Don't crash hard during import; routes will still work and show errors in logs.
...
def forgot_password():
    ...
    - Always respond with a neutral message (do not reveal whether the email exists).
    - Generate a one-time token with expiry and send a reset link via email.
    """
    if request.method == "POST":
        email = request.form.get("email", "").strip().lower()
        if not email:
        ...
        # Always show the same message (security), even if the email is unknown.
        if not user:
            db.close()
            flash("Wenn die E-Mail existiert, wurde ein Reset-Link versendet.", "ok")
        ...
        # Reset-Link per E-Mail versenden (kein Link mehr auf der Website).
        base_url = os.environ.get("RESET_BASE_URL", "https://umrahreisen.com").rstrip("/")
        reset_url = base_url + url_for("reset_password", token=token)

        try:
            send_reset_email(email, reset_url)
        except Exception:
            # Do not leak details to users; log for debugging.
...
def register():
    if request.method == "POST":
        ...
        try:
            ...
            # username or email already exists (UNIQUE)
