...
def translate_ar_to_de(text):
    MAP = {
        "اشترى الماء": "Wasser gekauft",
        "وجبة": "Mahlzeit",
        "نقل": "Transport",
    }
...
def dashboard():
    ...
    if role == "buchhaltung":
        if request.method == "POST":
            ...
            if action in ("approve", "deny") and kid:
                
conn = get_db()
cur = conn.cursor()
genehmigt_val = 1 if action == "approve" else -1

# Lade Datensatz, um Rückbuchungen korrekt zu handhaben
if DATABASE_URL:
...
if row:
    # dict-row support (sqlite row_factory) vs tuple
    if hasattr(row, "keys"):
    ...
    # Rückbuchung nur dann, wenn sich der Status auf 'abgelehnt' ändert (und vorher nicht abgelehnt war)
    if genehmigt_val == -1 and alt_status != -1:
        if DATABASE_URL:
            cur.execute("UPDATE users SET startguthaben = startguthaben + %s WHERE id=%s", (betrag, reisefuehrer_id))
        else:
            cur.execute("UPDATE users SET startguthaben = startguthaben + ? WHERE id=?", (betrag, reisefuehrer_id))

    # Wenn eine vorher abgelehnte Ausgabe nachträglich genehmigt wird, Rückbuchung wieder abziehen (Korrektur)
...
flash("Gespeichert", "ok")
        ...
        cur.execute(
            """
            SELECT k.id, k.user_id, u.username, u.startguthaben, k.datum, k.kategorie_ar, k.kategorie_de, k.beschreibung_ar, k.betrag_sar,
...
r = dict(row) if hasattr(row, "keys") else {
    ...
    "kategorie_ar": row[5],
    "kategorie_de": row[6],
    "beschreibung_ar": row[7],
    "betrag_sar": row[8],
    "beleg": row[9],
    "genehmigt": row[10],
    "genehmigt_von": row[11],
}
# Anzeige für Buchhaltung: Kategorie/ Beschreibung auf Deutsch
r["kategorie_anzeige"] = r.get("kategorie_de") or CATEGORIES.get(r.get("kategorie_ar"), {}).get("de") or r.get("kategorie_ar")
r["beschreibung_anzeige"] = translate_ar_to_de(r.get("beschreibung_ar") or "")
# Merker: Abgelehnt = zurückgebucht (Startguthaben wurde angepasst)
r["zurueckgebucht"] = True if r.get("genehmigt") == -1 else False
    ...
    if request.method == "POST":
        ...
        if action == "add_kosten":
            ...
            if DATABASE_URL:
                cur.execute(
                    """
                    INSERT INTO kosten (user_id, datum, kategorie_ar, kategorie_de, beschreibung_ar, beschreibung_de, betrag_sar, beleg, ohne_beleg, genehmigt, created_at)
                ...
                cur.execute(
                    """
                    INSERT INTO kosten (user_id, datum, kategorie_ar, beschreibung_ar, beschreibung_de, betrag_sar, beleg, ohne_beleg, genehmigt, created_at)
                    VALUES (?,?,?,?,?,?,?,?,?)
                    """,
                    (uid, datum, kategorie_ar, beschreibung_ar, betrag, beleg_filename, ohne_beleg, 0, _now_iso())
                )
        ...
        if action == "delete_kosten":
            ...
            if kid:
                if DATABASE_URL:
                    cur.execute("DELETE FROM kosten WHERE id=%s AND user_id=%s AND genehmigt=0", (kid, uid))
                else:
                    cur.execute("DELETE FROM kosten WHERE id=? AND user_id=? AND genehmigt=0", (kid, uid))
                conn.commit()
            conn.close()
            flash("Gelöscht", "ok")
            return redirect(url_for("dashboard"))
