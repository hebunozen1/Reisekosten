...
cur.execute(
    "SELECT 1 FROM users WHERE LOWER(username)=LOWER(%s)" if DATABASE_URL
    else "SELECT 1 FROM users WHERE LOWER(username)=LOWER(?)",
    (username,)
)
if cur.fetchone():
    ...
    return redirect(url_for("register"))
        ...
        if cur.fetchone():
            flash("Der Benutzername ist bereits vorhanden.")
            conn.close()
            return redirect(url_for("register"))
