...
def register():
    if request.method == "POST":
        ...
        try:
            send_registration_request_email(email, username, role)
            flash("Registrierungsanfrage gesendet. Du kannst dich nach Freigabe anmelden.", "ok")
        except Exception as exc:
            app.logger.exception("Approval request e-mail could not be sent for %s", email)
            flash(f"Registrierungsanfrage gespeichert. Die Buchhaltung sieht sie im System. E-Mail-Hinweis konnte nicht versendet werden: {exc}", "err")
        return redirect(url_for("login"))
...
def export_excel():
    ...
    try:
        ...
        writer.writerow(["ID", "Reiseführer", "Datum", "Reise", "Kategorie", "Beschreibung", "Betrag (SAR)", "Status", "Beleg"])
