...
def admin_delete_user(user_id):
    # Zugriff nur für Buchhaltung oder Admin
    if "user_id" not in session or session.get("role") not in ("buchhaltung", "admin"):
        flash("Keine Berechtigung.")
        return redirect(url_for("dashboard"))

    # Buchhaltung darf sich nicht selbst löschen
    if session.get("user_id") == user_id:
        flash("Eigenen Account kannst du hier nicht löschen.")
        return redirect(url_for("dashboard"))
