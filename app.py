...
def dashboard():
    ...
    if request.method == "POST":
        ...
        if action == "add_kosten":
            ...
            beleg_filename = None
        wechselkurs_filename = None
            beleg_file = request.files.get("beleg")
        wechselkurs_file = request.files.get("wechselkurs_beleg")
            
if beleg_file and beleg_file.filename:
    beleg_filename = secure_filename(beleg_file.filename)
    beleg_file.save(os.path.join(UPLOAD_FOLDER, beleg_filename))

if wechselkurs_file and wechselkurs_file.filename:
    wechselkurs_filename = secure_filename(wechselkurs_file.filename)
    wechselkurs_file.save(os.path.join(UPLOAD_FOLDER, wechselkurs_filename))
