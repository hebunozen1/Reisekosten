...
def translate_and_store_missing_descriptions(cur, kosten_rows):
    """Translate missing German descriptions and persist them, so exports use German text too."""
    changed = 0
...
def dashboard():
    ...
    try:
        if role == "buchhaltung":
            ...
            if translate_and_store_missing_descriptions(cur, kosten_all):
                conn.commit()
...
def export_excel():
    ...
    try:
        if role == "buchhaltung":
            ...
            if translate_and_store_missing_descriptions(cur, kosten_rows):
                conn.commit()
