...
def dashboard():
    ...
    if role == "buchhaltung":
        ...
        cur.execute(
            """
            SELECT k.id, k.user_id, u.username, k.datum, k.kategorie, k.beschreibung_ar, k.betrag_sar,
        ...
        for row in (cur.fetchall() or []):
            r = dict(row) if hasattr(row, "keys") else {
                "id": row[0], "user_id": row[1], "username": row[2], "datum": row[3], "kategorie": row[4],
    ...
    if request.method == "POST":
        ...
        if action == "add_kosten":
            ...
            if DATABASE_URL:
                cur.execute(
                    """
                    INSERT INTO kosten (user_id, datum, kategorie, beschreibung_ar, betrag_sar, beleg, ohne_beleg, genehmigt, created_at)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,0,NOW())
                    """,
                    (uid, datum, kategorie, beschreibung_ar, betrag, beleg_filename, ohne_beleg)
                )
            else:
                cur.execute(
                    """
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?)
                    """,
                    (uid, datum, kategorie, beschreibung_ar, betrag, beleg_filename, ohne_beleg, 0, _now_iso())
                )
...
def export_excel():
    ...
    cur.execute(
        """
        SELECT k.id, u.username, k.datum, k.kategorie, k.beschreibung_ar, k.betrag_sar, k.genehmigt
        FROM kosten k
        JOIN users u ON u.id = k.user_id
        ORDER BY k.id DESC
        """
