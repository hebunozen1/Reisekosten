...
def restore_belege():
    ...
    try:
        ...
        with zipfile.ZipFile(tmp_upload_path, "r", allowZip64=True) as zipf:
            ...
            if bad_file:
                raise RuntimeError(f"ZIP-Datei ist beschädigt bei: {bad_file}")

            for member in zipf.infolist():
                name = member.filename

                if member.is_dir() or not name.startswith("uploads/"):
                    skipped += 1
                    continue

                rel_name = posixpath.normpath(name[len("uploads/"):]).lstrip("/")
                if not rel_name or rel_name.startswith("../") or rel_name == "..":
                    skipped += 1
                    continue
        ...
        app.logger.info("Beleg-Backup wiederhergestellt: %s Dateien, %s übersprungen", restored, skipped)
