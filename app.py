...
def dashboard():
    ...
    if role == "buchhaltung":
        if request.method == "POST":
            ...
            if action in ("approve", "deny") and kid:
                ...
                if DATABASE_URL:
                    cur.execute(
                        "UPDATE kosten SET genehmigt=%s, genehmigt_von=%s, genehmigt_am=NOW() WHERE id=%s",
                        (genehmigt_val, session.get("username", ""), kid)
                    )
                else:
                    cur.execute(
                        "UPDATE kosten SET genehmigt=?, genehmigt_von=?, genehmigt_am=? WHERE id=?",
                        (genehmigt_val, session.get("username", ""), _now_iso(), kid)
                    )
                conn.commit()
                conn.close()
        ...
        if action == "add_kosten":
            ...
            if DATABASE_URL:
                ...
                cur.execute(
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?,?)
                    """,
                    (uid, datum, kategorie_ar, beschreibung_ar, beschreibung_de, betrag, beleg_filename, ohne_beleg, 0, _now_iso())
                )
    ...
    cur.execute(
        "SELECT id, datum, kategorie_ar, beschreibung_ar, beschreibung_de, betrag_sar, beleg, genehmigt FROM kosten WHERE user_id=%s ORDER BY id DESC" if DATABASE_URL else
