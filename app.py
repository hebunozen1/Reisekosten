...
# --- Rückbuchung bei Ablehnung durch Buchhaltung ---
@app.route("/reject_expense/<int:expense_id>")
def reject_expense(expense_id):
    ...
    if expense and expense["status"] != "abgelehnt":
        # Guthaben zurückbuchen
        cur.execute(
            "UPDATE users SET guthaben = guthaben + ? WHERE id = ?",
            (expense["betrag"], expense["user_id"])
        )
        # Status setzen
        cur.execute(
            "UPDATE expenses SET status = 'abgelehnt' WHERE id = ?",
            (expense_id,)
        )
        conn.commit()

    conn.close()
    flash("Ausgabe wurde abgelehnt und Betrag zurückgebucht.")
