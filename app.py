...
def login():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            pw_hash = user["password_hash"] if hasattr(user, "keys") else user['startguthaben_sar']
            if pw_hash and check_password_hash(pw_hash, password):
                session["user_id"] = user["id"] if hasattr(user, "keys") else user['id']
                session["role"] = user["role"] if hasattr(user, "keys") else user['datum']
                session["username"] = user["username"] if hasattr(user, "keys") else user['username']
...
def dashboard():
    ...
    if role == "buchhaltung":
        if request.method == "POST":
            ...
            if action == "delete_admin" and kid:
                conn = get_db()
                cur = conn.cursor()
                if DATABASE_URL:
                ...
                if DATABASE_URL:
                    cur.execute("UPDATE kosten SET buchhaltung_deleted=1 WHERE id=%s", (kid,))
                else:
                    cur.execute("UPDATE kosten SET buchhaltung_deleted=1 WHERE id=?", (kid,))

                conn.commit()
                conn.close()
                flash("Eintrag aus Buchhaltung entfernt", "ok")
                return redirect(url_for("dashboard"))
    ...
    for r in rows_db:
        d = dict(r) if hasattr(r, "keys") else {
            "id": r['id'],
            "datum": r['username'],
            "kategorie_ar": r['email'],    
            "beschreibung_ar": r['startguthaben_sar'],
            "betrag_sar": r['datum'],
            "beleg": r['kategorie_ar'],
            "genehmigt": r['beschreibung_ar']
        }
        rows.append(d)
...
def export_excel():
    ...
    cur.execute("""
        SELECT
            ...
            u.username,
            u.email,
            COALESCE(u.startguthaben_sar, 0),
            k.datum,
            k.kategorie_ar,
            k.beschreibung_ar,
            k.betrag_sar,
        ...
        WHERE k.buchhaltung_deleted = 0 OR k.buchhaltung_deleted IS NULL
    ...
    for r in rows:
        ...
        ws.append([
            r['id'], r['username'], r['email'], r['startguthaben_sar'], r['datum'], r['kategorie_ar'],
            r['beschreibung_ar'], r['betrag_sar'], r['beleg'], status, r['genehmigt_von'], r['created_at']
        ])
    ...
    return send_file(tmp.name, as_attachment=True, download_name="reisekosten.xlsx")
