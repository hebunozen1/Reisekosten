    ...
    try:
        with zipfile.ZipFile(file) as zipf:
            ...
            for member in zipf.infolist():
                ...
                # Wichtig: nicht source.read() komplett in den Speicher laden,
                # weil große Backup-ZIPs auf Render sonst hängen oder abbrechen können.
                with zipf.open(member) as source, open(target_path, "wb") as target:
                    shutil.copyfileobj(source, target, length=1024 * 1024)

                restored += 1

        app.logger.info("Beleg-Backup wiederhergestellt: %s Dateien", restored)
